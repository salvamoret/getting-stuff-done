...
class CreateCommand extends CommandBase
{
	...
	public function fire()
	{
		...
		{
			throw new \InvalidArgumentException( 'Must specify +name if title or subtitle used' );
		}
		else if ( $name[0] != '+' )
		{
			throw new \InvalidArgumentException( 'The list name must begin with a plus (+)' );
		}
		else
		{
			$name = substr( $name, 1 );
			...
			{
				throw new \InvalidArgumentException( "The list '$name' already exists" );
			}
		}
